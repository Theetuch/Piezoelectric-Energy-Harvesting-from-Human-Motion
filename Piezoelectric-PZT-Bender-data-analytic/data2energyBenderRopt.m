function [energy_avg, sum_Energy_avg, power_avg, time] = data2energyBenderRopt(data,r)
    ...
    %for various resistor, window_size = 2000, start = win/10, end = win*1
    %for various displacement, window size = 2000, start = win/5, end = win*8
    window_size = 2000; %assume the window size. can vary until it cover full pulse

    start_index1 = peak1_loc - window_size/5;
    start_index2 = peak2_loc - window_size/5;
    start_index3 = peak3_loc - window_size/5;

    end_index1 = peak1_loc + window_size*8;
    end_index2 = peak2_loc + window_size*8;
    end_index3 = peak3_loc + window_size*8;
